<?php
class Admin_Controller extends N2_Controller {
    ...
    function __construct()
    {
        ...
        if(!$user_is_logged_in){
            ...
            exit('No allowed to Admin section');
        }

        $_current_user = get_current_site_user();
        $_rolehaye_user = json_decode($_current_user->id_roleha);
        if( (!in_array('admin_user', $_rolehaye_user)) and (!in_array('super_user', $_rolehaye_user))){
            user_logout();
            redirect('site_core/user/login');
            exit('No allowed to Admin section');
        }
